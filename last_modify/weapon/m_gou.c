...
int do_duanlian(object weapon)
{
	...
	message_vision(HIR "$N手捏钩尖，一股内力丝丝的传了进去。\n" NOR, me);
	...
	if( w_zhi >= ((w_level + 1) * (w_level + 1)))
	{
		...
		message_vision(HIY "钩身忽的一亮，一道金光隐入$N的"+w_name+"，不见了！\n" NOR,me);
...
{	
	...
	me=this_player();
	w_name=me->query("weapon/name");
	if( !weapon || weapon != me->query("weapon/id"))
		return notify_fail("你要摧毁什么?\n");
	message_vision(HIR "$N大喝一声，一手擎钩，一手猛的一扯。结果啪的一声爆响"
			+w_name+HIR"断为两截!\n" NOR,me);
