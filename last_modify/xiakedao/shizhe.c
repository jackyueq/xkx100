...
// Modified by Zeratul Jan 11 2001 使者是上侠客岛接应者，呵呵
...
int ask_for_xkd()
{
	...
	{
		message_vision( "黄衣使者说道：上侠客岛要有赏善罚恶令，这位" 
			+ RANK_D->query_respect(who) + "还是请回吧！\n", who );
		return 1;
	}
	...
	{
		message_vision( "黄衣使者说道：" + RANK_D->query_respect(who) 
			+ "拿的不是自己的牌子。这样欺骗我，不是想害我家破人亡吗？\n", who );
		return 1;
	}
	...
	for(i=0; i<sizeof(inv); i++)
		if ( userp(inv[i]) )
		{
			message_vision( "黄衣使者说道：岛主颁下严令，只能迎接" + RANK_D->query_respect(who)
