...
int do_ride(string arg)
{
	...
	int roomi, roomttl, roadi, roadttl, this, direct;
...
// direct = 1 正向走, direct = 2 反向走, direct = 3 马夫位置不对, 无效
	...
	if (direct == 1)
	{
		...
		{
			ob->move(road[this]["room"][roomi]);
			tell_room(environment(ob),ob->query("name")+"骑着"+horse+"一路招摇疾驰而过。\n", ({ob}));
		}
		ob->move(road[this]["to_file"]);
		tell_room(environment(ob),ob->query("name")+"骑着"+horse+"一路顺利到达"+road[this]["to_name"]+"。\n");
	}
	else
	{
		...
		ob->move(road[this]["from_file"]);
		tell_room(environment(ob),ob->query("name")+"骑着"+horse+"一路顺利到达"+road[this]["from_name"]+"。\n");
	}
